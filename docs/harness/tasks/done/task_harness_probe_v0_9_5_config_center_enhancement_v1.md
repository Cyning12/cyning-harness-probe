...
- [x] `HarnessConfig` / `ExecutorConfig` / `SandboxConfig` / `AuditConfig` / `SafetyRefConfig` 模型完整
- [x] Pydantic 模型能自动校验类型、枚举、插件路径格式
- [x] `ConfigManager.default(env="test")` 正确加载 `config/*.test.yaml` 覆盖
- [x] `HARNESS_ENV=prod` 或 `--env prod` 能切换环境
- [x] `watch()` 在文件修改后自动重载配置
- [x] 重载后触发已注册的回调（如 `SafetyConfig.reload()`）
- [x] `harness-probe config validate` 输出配置错误
- [x] `harness-probe config show` 输出合并后配置
- [x] `pytest tests/ -q` 全绿
- [x] `ruff check .` 全绿
- [x] `mypy harness_sdk` 全绿
- [x] 图谱与 CHANGELOG 同步更新
- [x] 创建并推送 `task/v0-9-5-config-center-enhancement` 分支，合并到 `main`
...
| `watchdog` 依赖 | 需新增 | 已升级为必装依赖；降级逻辑保留用于极端场景 |
...
- 新增 `harness_sdk/config_models.py` 定义 Pydantic v2 配置模型。
- `ConfigManager` 新增 `env` 参数、多环境文件加载、`watch()`/`stop_watch()`/`register_on_reload()` 热重载接口。
- CLI 新增 `config validate` / `config show` / `config watch`，`run`/`compile`/`safety`/`config` 子命令统一支持 `--env`。
- `watchdog>=4.0` 加入 `pyproject.toml` 必装依赖。
- 新增 `config/executor.test.yaml` 多环境示例与 `docs/_tech_graph/87_config_center_enhancement.graph.yaml` 图谱。
